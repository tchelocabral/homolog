...
@section('content')
 ...
 <div class="row largura80 centralizado">
        <h1 class="margemB40">Editar Plano</h1>

        <form id="form-planos" name="form-planos" action="{{ route('planos.update', encrypt($plano->id)) }}" method="POST" enctype="multipart/form-data">
            @csrf
            {{method_field('PATCH')}}
            <div class="row">
                <div class="box box-solid box-primary com-shadow">
                    <div class="box-header th-ocean com-borda">
                        <h3 class="box-title">Dados da Plano</h3>
                    </div>
                    <div class="box-body box-profile">
                        <div class="col-md-12">
                            ...
                            <div class="row div-item-form">
                                <div class="col-md-6">
                                    <p class=""><b>Nome</b></p>
                                    <input type="text" name="name" class="form-control" value="{{ $plano->name }}" placeholder="Digite aqui" />
                                </div>
                            </div>
                                ...
                                <div class="col-md-12">
                                    ...
                                    @foreach($permissions as $per)
                                        @if($per->bloco != $current) 
                                            ...
                                            <div class="row">
                                                <div class="col-md-12">
                                                    ...
                                                    <h4 class="">
                                                        <a class="larguraTotal alturaTotal texto-preto" data-toggle="collapse" data-target="#{{ ucfirst($per->bloco) }}" aria-expanded="false" aria-controls="#collapsejobrecu" plano="button"><span class="accordion-marc"><i class="fa fa-angle-right"></i></span>Permissões de {{ ucfirst($per->bloco) }}</a>
                                                    </h4>
                                        ...
                                        <div class="col-md-4 margemT10"> 
                                            ...
                                            {{ in_array($per->id, $planoPermissions) ? 'checked="checked' : '' }}
                                            />
                                            {{ $per->name }}
                                        </div>
                                    @endforeach
                    ...
                    <div class="box-footer">
                        <button type="submit" class="btn btn-primary pull-right">Atualizar Plano</button>
                    </div>
                </div>
            </div>
        </form>
