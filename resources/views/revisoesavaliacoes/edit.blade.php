...
<div class="row paddingB40">
    ...
    <div class="col-md-12 col-sm-12">
        <form action="{{ route('update.revisao.avaliacao', encrypt($revisao->id)) }}" method="POST" enctype="multipart/form-data" id="form-revisao-edita">
            <div>
                <div>
                            ...
                            <div class="row" id="RowPinX{{$index+1}}">
                                <div class="col-md-3">
                                    <div class="largura95 divTxtPonto">
                                        <textarea class="form-control image-preview-filename" required="true" name="pins[{{ $index+1 }}][texto]" id="txtPontoDesc{{ $marc->id }}" style="height:100px" rows="5">{{ $marc->texto }}</textarea>
                                        <input type="hidden" name="xPin" id="xPin{{ $index }}" value="{{ $marc->x }}">
                                        <input type="hidden" name="yPin" id="yPin{{ $index }}" value="{{ $marc->y }}">

                                        <input type="hidden" name="pin_flag" id="pin-flag-{{$index+1}}" data-origin="banco"  value="banco" data-url="{{route('deletar.pin.revisao.avaliacao', encrypt($marc->id))}}">
                                    ...
                                    @forelse($marc->midias as  $indexMi => $mid)
                                        <div class="input-group image-preview margemB5" data-original-title="" title="">
                                            ...
                                            <input class="form-control image-preview-filename largura90" 
                                                ...
                                                data-url="{{route('deletar.midia.pin.revisao.avaliacao', 
                                            ...
                                            <span class="input-group-btn largura ">
                                                ...
                                                @if ($indexMi<1)
                                                    <button type="button" class="btn btn-success" style="margin-left: 0px;" title="{{ __('messages.Adicionar Arquivo') }}" id="bt-add-arquivo-{{$index+1}}-{{$indexMi+1}}"> + </button>
                                                @endif
                                       ...
                                       <div class="input-group image-preview margemB5" data-original-title="" title="">
                                            ...
                                            <span class="input-group-btn largura ">
                                                ...
                                                <button class="btn btn-default image-preview-clear" 
                                                    ...
                                                    <span class="glyphicon glyphicon-remove"> {{ __('messages.Limpar') }}</span>
                                                </button>
                                                <button type="button" class="btn btn-success" style="margin-left: 0px;" title="Adicionar Arquivo" id="bt-add-arquivo-{{$index+1}}-1"> + </button>
