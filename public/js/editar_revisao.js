...
var formRevisao = document.getElementById("form-revisao-edita");

//pega o id da div areaInput
var divAreaInput = document.getElementById('areaInput');

var tempXY = document.getElementById('tempXY');

//pega o id da imagem do pin e passa o caminho dos pins em png
var imgPin = document.getElementById('imagem_pin_edita');
...
function renderSceneEditar() {

    requestAnimationFrame(renderSceneEditar);

    // pega o elemento canvas
    var canvasEdita  = document.getElementById("canvas_revisao_edita_img");
    // chama o metodo contexto 2D
    var contextEdita = canvasEdita.getContext('2d');

    eventsCanvas(canvasEdita);



    var imgBg = new Image();
    
    //imgBg.src =  url_base + "/images/" + imgSectionEdita;
    imgBg.src =   imgSectionEdita.src;
    ...
    canvasEdita.width = imgBg.width;
    canvasEdita.height = imgBg.height;

    // limpa o canvas
    contextEdita.clearRect(0,0,
        canvasEdita.width,
        canvasEdita.height
    );
    ...
    contextEdita.drawImage(imgBg, 4, 4, imgBg.width, imgBg.height);

    for(var x = 0,len = imagesOnCanvasEdita.length; x < len; x++) {

        var obj = imagesOnCanvasEdita[x];

        contextEdita.drawImage(obj.image,obj.x,obj.y);
...
function startMove(obj,downX,downY) {
    ...
    // pega o elemento canvas
    var canvasEditar = document.getElementById('canvas_revisao_edita_img');
    ...
    canvasEditar.onmousemove = function(e) {
    ...
    canvasEditar.onmouseup = function() {

        canvasEditar.onmousemove = function(){};
    }
...
function drop(e) {
    ...
    e.preventDefault();
    var pinAtual = imagesOnCanvasEdita.length+1;
    ...
    var tempArea =  "";
    tempXY.innerText = "";
    for (i = 0; i < imagesOnCanvasEdita.length; i++) {

        tempArea+= imagesOnCanvasEdita[i].image.id+ " - ";;
    }

    tempXY.innerText =  tempArea ;
...
//    txtPontoAtual.value = pinProx;
    ...
    var posTempX = e.clientX - canvasEdita.offsetLeft - mouse_position_x;
    var posTempY = e.clientY - canvasEdita.offsetTop - mouse_position_y;

    //criar estrutura para salvar no banco id ponto, x, y e texto;
    createInputPonto(imagesOnCanvasEdita.length, posTempX, posTempY, imagesOnCanvasEdita.length);
...
function loadPin()
{
    ...
    imgPin.src = url_base + "/images/pins/azul/" + (imagesOnCanvasEdita.length+1) + ".png";
...
function loadImage()
{
    imgSectionEdita = document.getElementById("img_final_edita");
    requestAnimationFrame(renderSceneEditar);
    loadPin();
...
function eventsCanvas(canvasEdita)
{
    canvasEdita.addEventListener('click', () => {
       console.log('canvasEdita click');
    });
    
    // adiciona função no evento onmousedown do canvas
    canvasEdita.onmousedown = function(e) {
      //  alert("mouse down");
    ...
    canvasEdita.ondblclick = function(e) {
        ...
        // percorre todas as imagens do canvas
         for(var x = 0,len = imagesOnCanvasEdita.length; x < len; x++) {
           ...
           // pega o obj da posiçao x
            ...
            alert(obj.id  +" - " + obj.image.id +" - " + imagesOnCanvasEdita.length);
            // senao caiu no IF, inicia o movimento
            var r = confirm("Você quer deletar esse ponto?");
            if(r==true) {

                if(obj.image.id < imagesOnCanvasEdita.length)
                {
                ...
                else
                {
                    ...
                    imagesOnCanvasEdita.splice(x,1);
 ...
 //       txtPontoAtual.value = imagesOnCanvasEdita.length+1;
        imgPin.src = url_base + "/images/pins/azul/" + (imagesOnCanvasEdita.length+1) + ".png";
